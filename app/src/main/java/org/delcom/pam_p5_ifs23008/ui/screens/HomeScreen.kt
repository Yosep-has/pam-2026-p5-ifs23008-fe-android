...
import org.delcom.pam_p5_ifs23008.ui.components.StatusCard
...
import org.delcom.pam_p5_ifs23008.ui.theme.DelcomTheme
import org.delcom.pam_p5_ifs23008.ui.viewmodels.AuthActionUIState
...
import org.delcom.pam_p5_ifs23008.ui.viewmodels.TodoStatsUIState
...
fun HomeScreen(
    ...
    var stats by remember { mutableStateOf<ResponseTodoStatsData?>(null) }
            ...
            if (uiStateAuth.auth is AuthUIState.Success) {
                if (isFreshToken) {
                    ...
                    if (authToken != newToken) {
                        authToken = newToken
                        // Muat statistik todo setelah token siap
    ...
    // Tangkap hasil statistik todo
    LaunchedEffect(uiStateTodo.todoStats) {
        if (uiStateTodo.todoStats is TodoStatsUIState.Success) {
            stats = (uiStateTodo.todoStats as TodoStatsUIState.Success).data
        }
    }
...
fun PreviewHomeUI() {
    DelcomTheme {
        HomeUI(
            stats = ResponseTodoStatsData(total = 10, done = 6, notDone = 4)
        )
    }
}